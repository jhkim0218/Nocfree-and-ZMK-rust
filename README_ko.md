...
> [!IMPORTANT]
> 배터리 퍼센트 표기는 아직 보정되지 않았고, 물리 스위치 옆 상태 LED 제어는
> 순정 동작을 정확히 재현하지 못합니다. 공장 USB 동글/2.4G 기능도 구현·검증되지
> 않았습니다.
...
| 미구현 | 물리 스위치 옆 LED | 표시와 제어가 순정 firmware와 정확히 일치하지 않음. 빨간 충전/저전압 LED와 파란 상태 LED의 순정 동작 재현이 남아 있음 |
