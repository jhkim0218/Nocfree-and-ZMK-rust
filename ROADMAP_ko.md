...
| Rust 왼쪽 ANSI | 138,240 | `0x27000..0x37DFF` |
...
| P0 회귀 | split 재연결 | 반쪽을 가까이 옮기지 않고 일반 책상 거리에서 오른쪽 재연결 | 약 30 cm에서 실패하고 가까이 가져가야 연결되는 경우가 있음 | 진단으로 실패 단계를 확인하고 책상 거리 반복 재연결 통과 |
| P0 회귀 | 백라이트 동기화 | 명령·timeout·wake·재부팅·재연결 뒤 양쪽 상태 일치 | 상대 toggle 명령이 좌우 반전 상태를 계속 유지할 수 있음 | 왼쪽 기준 절대 상태가 일부러 어긋낸 오른쪽을 동기화 1회로 수렴 |
