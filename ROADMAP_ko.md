...
| Experimental | Rust USB 동글 | 왼쪽의 병합 report를 하나의 공용 USB receiver로 전달 | 암호화 BLE service, 전용 bond, 고정 최대 크기 report, disconnect release, USB HID가 소프트웨어 검사를 통과. 공용 이미지는 ANSI 실기 통과. 순정 ESB/nRF24/numpad는 범위 밖 | ISO/JIS/KR 대응 키보드 쌍도 pairing/reconnect/입력 순서/latency/recovery/coexistence 통과 |
