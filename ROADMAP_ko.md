...
| Rust 왼쪽 ANSI | 150,528 | `0x27000..0x395FF` |
| Rust 오른쪽 ANSI | 93,696 | `0x27000..0x326FF` |
...
| P0 회귀 | 좌우 입력 순서 | 누락·중복·고착 없이 물리적인 좌우 이벤트 순서 보존 | 반쪽별 snapshot을 도착 순서로 처리해 빠른 교차 입력에서 `jam -> ajm` 발생 가능 | 10,000회 이상 합성 이벤트와 USB/BLE 실기 스트레스가 허용 가능한 추가 지연으로 통과 |
| P0 회귀 / P3.1 완료 | split 재연결 | 반쪽을 가까이 옮기지 않고 일반 책상 거리에서 오른쪽 재연결 | 표준 ATT MTU 23으로 관측된 MTU 교환 실패 제거. 사용자가 승인한 책상 거리 2회는 입력까지 복귀했지만 split-ready가 17.7-55.4초 걸림 | TX 출력/latency는 유지하고 단계별 광고와 미연결 키 fast 광고부터 시험한 뒤 책상 거리 반복 |
