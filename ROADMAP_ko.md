...
| 후보 | 좌우 입력 순서 | 누락·중복·고착 없이 물리적인 좌우 이벤트 순서 보존 | 원본 timestamp·순번·시계 변환·8 ms 전역 대기열로 도착 순서 의존 제거; 8 ms는 실기 미검증 | 실제 jitter 측정과 runtime queue 전체 10,000회 검증, Wired/Windows 11/Android 및 재연결 직후 스트레스 통과 |
...
3. **P4.3 실기:** Wired, Windows 11 Bluetooth, Android, 재연결 직후와 양쪽
   1200-baud 복구를 반복합니다. 측정이 뒷받침할 때만 8 ms를 유지합니다.
