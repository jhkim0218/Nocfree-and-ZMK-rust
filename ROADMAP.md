...
| Candidate | Cross-half ordering | Physical left/right event order is preserved without lost, duplicate, or stuck keys | Source timestamps, sequence, clock conversion, and a 3 ms global queue replace arrival-only ordering | Measure real jitter and prove 10,000+ events end to end through runtime queues; repeat Wired/Windows 11/Android and reconnect-edge stress |
...
3. P4.3 repeat Wired, Windows 11 Bluetooth, Android, reconnect-edge, and both
   1200-baud recovery checks. Retain 3 ms only if measurements support it.
